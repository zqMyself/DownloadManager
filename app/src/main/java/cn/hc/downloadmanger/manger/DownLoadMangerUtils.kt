 ...
 */
class DownLoadMangerUtils {

    var downloadManager :DownloadManager?=null
//    var downLoadBroadcastReceiver : DownLoadBroadcastReceiver?=null
    var idMap = HashMap<Long,String>()
    ...
    companion object{
        var mDownLoadMangerUtils: DownLoadMangerUtils? = null
        fun getInstance(): DownLoadMangerUtils {
            if (mDownLoadMangerUtils == null) {
                synchronized(DownLoadMangerUtils::class.java) {
                    if (mDownLoadMangerUtils == null) {
                        mDownLoadMangerUtils = DownLoadMangerUtils()
                    }
                }
            }
            return mDownLoadMangerUtils!!
        }
    }
    ...
    fun  notifyListener(id: Long,state:Int,progress:Int){

            if (idMap.containsKey(id)) {
                var url =idMap.get(id)!!
                mHandler.post {
...
//                    }
    ...
    fun downLoad(title:String,description:String,downLoadHtmlUrl:String){
        ...
        if (downloadManager != null) {
            ...
            startTimer(id,downLoadHtmlUrl)
        }

    }


    private fun startTimer(id:Long,downLoadHtmlUrl: String){
        if (!idMap.containsKey(id)){
            idMap[id] = downLoadHtmlUrl
            if (timer == null) {
                timer= Timer()
...
//                    Log.e("tag","STATUS_RUNNING +" + DownloadManager.STATUS_RUNNING)
                    ...
                    notifyListener(id, DownloadManager.STATUS_RUNNING, dl_progress.toInt())
                    Log.e("tag","STATUS_PENDING  bytesAndStatus = $dl_progress")
                }
                DownloadManager.STATUS_SUCCESSFUL -> {
                    notifyListener(id, DownloadManager.STATUS_SUCCESSFUL, 100)
                    if(idMap.containsKey(id)){
                        idMap.remove(id)
                    }
